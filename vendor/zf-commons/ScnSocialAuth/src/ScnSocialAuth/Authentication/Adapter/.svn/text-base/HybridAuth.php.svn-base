...
class HybridAuth extends AbstractAdapter implements ServiceManagerAwareInterface, EventManagerAwareInterface
{
        ...
        if (false == $localUserProvider) {
            ...
            if (method_exists($this, $method)) {
                ...
                try {
                    ...
                    $_SESSION['social_data'] = $userProfile;
                    $socialdataArray = get_object_vars($_SESSION['social_data']);
                    $socialEmail = $socialdataArray['email'];
                    $mapper = $this->getZfcUserMapper();
                    $countemail = $mapper->findByEmail($socialEmail);
                    if(count($countemail) == '0'){
                        return 'new user'; // by vikash to make social login/registration in sink with normal registration
                    }
    ...
    protected function facebookToLocalUser($userProfile)
    {
        ...
        $localUser = $this->instantiateLocalUser();
        $passwordArr = explode('@',$userProfile->emailVerified);
        $passwordConcat = '';
        ...
        $localUser->setEmail($userProfile->emailVerified)
            ...
            ->setUsername($userProfile->username?$userProfile->username:$userProfile->emailVerified)
            ->setPhone($userProfile->phone)
            ->setGender(ucwords($userProfile->gender))
            ->setUserTypeId(1)
            ->setAllowschedule(0)   
    ...
    protected function googleToLocalUser($userProfile)
    {
        ...
        $localUser = $this->instantiateLocalUser();
        $passwordArr = explode('@',$userProfile->email);
        $passwordConcat = '';
        if(strlen($passwordArr[0])<6){
            for($passVar = 1 ; $passVar <= 6-strlen($passwordArr[0]) ; $passVar++){
                $passwordConcat .= $passVar;  
    ...
    protected function insert($user, $provider, $userProfile)
    {
        ...
        $result = $this->getZfcUserMapper()->insertSocial($user);
