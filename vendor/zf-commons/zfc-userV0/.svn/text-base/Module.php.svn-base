        ...
        return array(
            'factories' => array(
                ...
                'zfcUserRegisterWidget' => function ($sm) {
                    ...
                    // to know whether south africa or india extramarks
                    $manager  = $locator->get('ModuleManager');
                    $modules  = $manager->getLoadedModules();
                    $loadedModules      = array_keys($modules);
                    $southafricaLoaded = in_array('Southafrica',$loadedModules);
                    // south africa ends here
                    
                    // ip work starts
                    $ipCountryTable = $locator->get('Assessment\Model\IpCountryTable');
                    $com_mapper = $locator->get('com_mapper');
                    $viewHelper->setIpCountryTableGateway($ipCountryTable,$com_mapper);
                    // ip work ends here
        
                    $viewHelper->setViewTemplate($locator->get('zfcuser_module_options')->getUserRegisterWidgetViewTemplate());
                    $viewHelper->setRegisterForm($locator->get('zfcuser_register_form') , $southafricaLoaded);
                    return $viewHelper;
                },        
            ),
        );
        ...
        return array(
            ...
            'factories' => array(
                ...
                'zfcuser_register_form' => function ($sm) {
                    ...
                    $com_mapper = $sm->get('com_mapper');
                    $lms_container_mapper = $sm->get('lms_container_mapper');
                    $options = $sm->get('zfcuser_module_options');
                    $form = new Form\ProfileBase($com_mapper,$lms_container_mapper);
                    //$form->setCaptchaElement($sm->get('zfcuser_captcha_element'));
                    $form->setInputFilter(new Form\ProfileFilter(
                        new Validator\NoRecordExists(array(
                            'mapper' => $sm->get('zfcuser_user_mapper'),
                            'key'    => 'email'
                        )),
