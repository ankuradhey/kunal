...
use ZfcUserV0\Authentication\Adapter\AdapterChainEvent as AuthEvent;
use ZfcUserV0\Mapper\User as UserMapperInterface;
use ZfcUserV0\Options\AuthenticationOptionsInterface;
...
class Db extends AbstractAdapter implements ServiceManagerAwareInterface
{
    ...
    public function authenticate(AuthEvent $e)
    {
        ...
        // By vikash [for authentication without password]
        if($e->getRequest()->getPost()->get('md5_credential') !== NULL )
            $credential = $e->getRequest()->getPost()->get('md5_credential');
        $userObject = null;
...
//        if (!$bcrypt->verify($credential, $userObject->getPassword())) {
        // By vikash [for authentication without password]
        if($e->getRequest()->getPost()->get('md5_credential') !== NULL ){
            if (($credential) != $userObject->getPassword()) {
            ...
            if (md5($credential) != $userObject->getPassword()) {
                // Password does not match
                $e->setCode(AuthenticationResult::FAILURE_CREDENTIAL_INVALID)
                  ->setMessages(array('Supplied credential is invalid.'));
                $this->setSatisfied(false);
